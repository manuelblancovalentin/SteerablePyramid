...
mMlambda = real(mMlambda(abs(tg)<1e-6));
...
elseif sk>=skmax & Warn,
    ...
    if length(lam)>1,
        ...
        if any(foo==0),
            ...
            lam = lam(foo==sign(sk));		% rejects the symmetric solution
        end
        if length(lam)>0,
            lam=lam(abs(lam)==min(abs(lam)));	% the smallest that fix the skew
            lam=lam(1);
        else
            lam = 0;
        end
